# Kaj Guide

The guide teaches current Kaj through runnable examples. For exact rules and diagnostics, follow each page's reference link.

## Core syntax

- [Variables](variables.md)
- [Types](types.md)
- [Operators](operators.md)
- [Control flow](control-flow.md)
- [Functions](functions.md)

## Data

- [Lists](lists.md)
- [Maps](maps.md)
- [Records](records.md)

## Tagged types and type safety

- [Enums and match](enums-and-match.md)
- [Optional and Result](optional-and-result.md)
- [Newtypes](newtypes.md)

## Programs

- [Modules](modules.md)
- [Kaj by Example](kaj-by-example.md)

Normative additions: [loop control and ranges](../language/loop-control-and-ranges.md), [strings and conversion](../language/strings-and-conversion.md), and [equality and display](../language/equality-and-display.md).
